...
internal class HeaderTests {
    ...
    fun `header with incorrect style validated properly`() {
        ...
        Assert.state(parser.errors[2].errorType == ErrorType.TEXT_HEADER_NOT_BOLD, "Wrong error!")
        Assert.state(parser.errors[3].errorType == ErrorType.TEXT_HEADER_NOT_UPPERCASE, "Wrong error!")
        Assert.state(parser.errors[4].errorType == ErrorType.TEXT_COMMON_FONT, "Wrong error!")
        Assert.state(parser.errors[5].errorType == ErrorType.TEXT_COMMON_INCORRECT_FONT_SIZE, "Wrong error!")
        Assert.state(parser.errors[6].errorType == ErrorType.TEXT_COMMON_TEXT_COLOR, "Wrong error!")
    }
}